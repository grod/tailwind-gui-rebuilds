<div class="flex h-screen items-center justify-center p-4 bg-gray-200">
    <div class="flex flex-col items-center">
        <h1 class="text-3xl font-semibold text-gray-800">GUI Rebuilds</h1>
        <div class="grid grid-cols-2 gap-4 mt-8 text-indigo-800">
                <a href="/spotify" class="flex justify-center px-4 py-4 rounded-full bg-white shadow-md hover:shadow-lg">
                    <img class="h-10" src="spotify-logo.png" alt="Spotify" />
                </a>
                <a href="/slack" class="flex justify-center items-center p-4 rounded-full bg-white shadow-md hover:shadow-lg">
                    <img src="slack-logo.png" class="h-10" alt="Slack" />
                </a>
                <a href="/unsplash" class="flex justify-center items-center p-4 rounded-full bg-white shadow-md hover:shadow-lg">
                    <img src="unsplash-logo.png" class="h-10" alt="Unsplash" />
                </a>
                <a href="/netlify" class="flex justify-center items-center p-4 rounded-full bg-white shadow-md hover:shadow-lg">
                    <svg class="w-10 h-10 text-teal-500 fill-current" viewBox="0 0 400 400">
                        <path id="logomark" d="M290 139l-1-1 8-50 38 38-39 17h-1l-5-4zm55-3l40 40c9 9 13 13 14 18l1 2-97-41-1-1 1-1 42-17zm53 73c-2 4-6 8-13 15l-45 45-59-12h-1l-1-1c0-5-3-9-7-12v-1l11-68v-1l1-1c5 0 9-3 12-7h2l100 43zm-69 71l-75 75 13-79 1-1 7-5 1-1 53 11zm-91 91l-8 8-94-135v-1l1-2 1-1h1l103 22h1v1c2 5 6 10 11 12v1l-16 95zm-17 17c-7 6-10 10-15 11h-12c-5-2-9-6-18-14l-94-94 25-38h1a25 25 0 0 0 18-1h1l94 136zM73 282l-21-21 42-19h1l1 1 1 2v1l-24 36zm-31-31l-27-27-10-11 83 17 1 1-1 1-46 19zM0 199l1-5c1-5 6-9 14-18l35-35a22695 22695 0 0 0 48 70l1 1a29 29 0 0 0-5 6h-1L0 199zm59-67l47-47 35 15a5977 5977 0 0 1 25 11l-1 5c0 5 2 10 6 14v2l-48 73v1h-1l-6-1-5 1h-1v-1l-51-73zm57-56l60-61c9-8 13-12 18-14h12c5 2 10 6 18 14l13 13-43 67v1h-1l-7-1c-5 0-10 1-13 4h-2l-55-23zm130-39l40 40-9 60h-1v1a19 19 0 0 0-6 3h-1l-61-26-1-1a23 23 0 0 0-3-9v-2l42-66zm-41 90l57 24 1 1v5l-1 1-127 54-1-1 1-1 47-72 1-1h3a21 21 0 0 0 18-10h1zm-65 96l128-55 1 1 1 1h1v1l-11 68v1c-6 0-12 4-15 9v1h-1l-102-22-2-5z"></path>
                    </svg>
                    <div class="ml-3 justify-center font-medium text-3xl text-gray-800">Netlify</div>
                </a>
                <a href="/github" class="flex justify-center items-center p-4 rounded-full bg-white shadow-md hover:shadow-lg">
                    <img src="github-logo.png" class="h-10" alt="GitHub" />
                </a>
                <a href="/playpilot" class="flex justify-center items-center p-4 rounded-full bg-white shadow-md hover:shadow-lg">
                    <img src="playpilot-logo.png" class="h-10" alt="Playpilot" />
                    <div class="ml-3 justify-center font-medium text-2xl text-gray-800">Playpilot</div>
                </a>
                <a href="/pitch" class="flex justify-center items-center p-4 rounded-full bg-white shadow-md hover:shadow-lg">
                    <div class="flex items-center h-10">
                        <svg fill="none" height="23" width="66"><path d="M44.87 17.272a4.318 4.318 0 11.04-4.877l3.191-1.884a8.019 8.019 0 10-.017 8.613l-3.213-1.852zM53.662 0H49.96v22.206h3.7v-7.402a4.318 4.318 0 118.636 0v7.402h3.701v-8.122a7.3 7.3 0 00-12.336-5.282V0zM27.756 0h-3.701v17.271a5.551 5.551 0 009.201 4.183l-1.788-3.034a1.847 1.847 0 01-1.45.701h-.412a1.851 1.851 0 01-1.85-1.85v-6.168h3.7V7.402h-3.7V0zM19.74 0a2.056 2.056 0 100 4.112 2.056 2.056 0 000-4.112zM21.59 22.206V7.402h-3.7v14.804h3.7z" fill="currentColor"></path><path clip-rule="evenodd" d="M0 22.206h3.7v-4.318h3.393A8.944 8.944 0 107.093 0H0v22.206zM3.7 3.7h3.393a5.243 5.243 0 010 10.486H3.701V3.7z" fill-rule="evenodd" fill="currentColor"></path></svg>
                    </div>
                </a>
                <a href="/prismic" class="flex justify-center items-center p-4 rounded-full bg-white shadow-md hover:shadow-lg">
                    <img class="h-8" src="prismic-logo.png" alt="" />
                </a>
        </div>
    </div>
</div>